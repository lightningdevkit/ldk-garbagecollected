...
public class DecryptedOurPeerStorage extends CommonBase {
	DecryptedOurPeerStorage(Object _dummy, long ptr) { super(ptr); }
	 ...
	 */
	public static DecryptedOurPeerStorage of(byte[] data) {
		long ret = bindings.DecryptedOurPeerStorage_new(data);
	...
	public byte[] into_vec() {
		byte[] ret = bindings.DecryptedOurPeerStorage_into_vec(this.ptr);
	 ...
	 */
	public EncryptedOurPeerStorage encrypt(org.ldk.structs.PeerStorageKey key, byte[] random_bytes) {
		long ret = bindings.DecryptedOurPeerStorage_encrypt(this.ptr, key.ptr, InternalUtils.check_arr_len(random_bytes, 32));
		Reference.reachabilityFence(this);
		Reference.reachabilityFence(key);
		Reference.reachabilityFence(random_bytes);
		if (ret >= 0 && ret <= 4096) { return null; }
