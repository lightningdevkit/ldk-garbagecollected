using org.ldk.impl;
using org.ldk.enums;
using org.ldk.util;
using System;

namespace org { namespace ldk { namespace structs {


/**
 * A secret key used to authenticate message contexts in received [`BlindedMessagePath`]s.
 * 
 * This key ensures that a node only accepts incoming messages delivered through
 * blinded paths that it constructed itself.
 * 
 * [`BlindedMessagePath`]: crate::blinded_path::message::BlindedMessagePath
 */
public class ReceiveAuthKey : CommonBase {
	internal ReceiveAuthKey(object _dummy, long ptr) : base(ptr) { }
	~ReceiveAuthKey() {
		if (ptr != 0) { bindings.ReceiveAuthKey_free(ptr); }
	}

	public byte[] get_a() {
		long ret = bindings.ReceiveAuthKey_get_a(this.ptr);
		GC.KeepAlive(this);
		if (ret >= 0 && ret <= 4096) { return null; }
		byte[] ret_conv = InternalUtils.decodeUint8Array(ret);
		return ret_conv;
	}

	public void set_a(byte[] val) {
		bindings.ReceiveAuthKey_set_a(this.ptr, InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(val, 32)));
		GC.KeepAlive(this);
		GC.KeepAlive(val);
	}

	/**
	 * Constructs a new ReceiveAuthKey given each field
	 */
	public static org.ldk.structs.ReceiveAuthKey of(byte[] a_arg) {
		long ret = bindings.ReceiveAuthKey_new(InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(a_arg, 32)));
		GC.KeepAlive(a_arg);
		if (ret >= 0 && ret <= 4096) { return null; }
		org.ldk.structs.ReceiveAuthKey ret_hu_conv = null; if (ret < 0 || ret > 4096) { ret_hu_conv = new org.ldk.structs.ReceiveAuthKey(null, ret); }
		if (ret_hu_conv != null) { ret_hu_conv.ptrs_to.AddLast(ret_hu_conv); };
		return ret_hu_conv;
	}

	internal long clone_ptr() {
		long ret = bindings.ReceiveAuthKey_clone_ptr(this.ptr);
		GC.KeepAlive(this);
		return ret;
	}

	/**
	 * Creates a copy of the ReceiveAuthKey
	 */
	public org.ldk.structs.ReceiveAuthKey clone() {
		long ret = bindings.ReceiveAuthKey_clone(this.ptr);
		GC.KeepAlive(this);
		if (ret >= 0 && ret <= 4096) { return null; }
		org.ldk.structs.ReceiveAuthKey ret_hu_conv = null; if (ret < 0 || ret > 4096) { ret_hu_conv = new org.ldk.structs.ReceiveAuthKey(null, ret); }
		if (ret_hu_conv != null) { ret_hu_conv.ptrs_to.AddLast(this); };
		return ret_hu_conv;
	}

	/**
	 * Checks if two ReceiveAuthKeys contain equal inner contents.
	 * This ignores pointers and is_owned flags and looks at the values in fields.
	 * Two objects with NULL inner values will be considered "equal" here.
	 */
	public bool eq(org.ldk.structs.ReceiveAuthKey b) {
		bool ret = bindings.ReceiveAuthKey_eq(this.ptr, b.ptr);
		GC.KeepAlive(this);
		GC.KeepAlive(b);
		return ret;
	}

	public override bool Equals(object o) {
		if (!(o is ReceiveAuthKey)) return false;
		return this.eq((ReceiveAuthKey)o);
	}
}
} } }
