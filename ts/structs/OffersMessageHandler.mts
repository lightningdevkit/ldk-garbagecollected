...
export class OffersMessageHandler extends CommonBase {
	...
	public static new_impl(arg: OffersMessageHandlerInterface): OffersMessageHandler {
		...
		let structImplementation = {
			handle_message (message: bigint): bigint {
				...
				const ret: Option_OffersMessageZ = arg.handle_message(message_hu_conv);
				const result: bigint = ret == null ? 0n : ret.clone_ptr();
				CommonBase.add_ref_from(impl_holder.held, ret);
				return result;
			},
			release_pending_messages (): number {
				const ret: ThreeTuple_OffersMessageDestinationBlindedPathZ[] = arg.release_pending_messages();
				const result: number = bindings.encodeUint64Array(ret.map(ret_conv_49 => ret_conv_49 == null ? 0n : ret_conv_49.clone_ptr()));
				return result;
			},
	 ...
	 * [`OnionMessenger`]: crate::onion_message::messenger::OnionMessenger
	 */
	public handle_message(message: OffersMessage): Option_OffersMessageZ {
		const ret: bigint = bindings.OffersMessageHandler_handle_message(this.ptr, CommonBase.get_ptr_of(message));
		const ret_hu_conv: Option_OffersMessageZ = Option_OffersMessageZ.constr_from_ptr(ret);
	 ...
	 * another message. The latter should use the return value of [`Self::handle_message`].
	 */
	public release_pending_messages(): ThreeTuple_OffersMessageDestinationBlindedPathZ[] {
		...
		const ret_conv_49_arr: ThreeTuple_OffersMessageDestinationBlindedPathZ[] = new Array(ret_conv_49_len).fill(null);
		for (var x = 0; x < ret_conv_49_len; x++) {
			const ret_conv_49: bigint = bindings.getU64ArrayElem(ret, x);
			const ret_conv_49_hu_conv: ThreeTuple_OffersMessageDestinationBlindedPathZ = new ThreeTuple_OffersMessageDestinationBlindedPathZ(null, ret_conv_49);
