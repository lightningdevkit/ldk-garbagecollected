...
export interface PersisterInterface {
	 ...
	 */
	persist_manager(channel_manager: ChannelManager): Result_NoneIOErrorZ;
	/**Persist the given [`NetworkGraph`] to disk, returning an error if persistence failed.
	 */
	persist_graph(network_graph: NetworkGraph): Result_NoneIOErrorZ;
	/**Persist the given [`WriteableScore`] to disk, returning an error if persistence failed.
	 */
	persist_scorer(scorer: WriteableScore): Result_NoneIOErrorZ;
}
...
export class Persister extends CommonBase {
	...
	public static new_impl(arg: PersisterInterface): Persister {
		...
		let structImplementation = {
			persist_manager (channel_manager: bigint): bigint {
				...
				const ret: Result_NoneIOErrorZ = arg.persist_manager(channel_manager_hu_conv);
			...
			persist_graph (network_graph: bigint): bigint {
				...
				const ret: Result_NoneIOErrorZ = arg.persist_graph(network_graph_hu_conv);
				const result: bigint = ret == null ? 0n : ret.clone_ptr();
				return result;
			},
			persist_scorer (scorer: bigint): bigint {
				...
				const ret: Result_NoneIOErrorZ = arg.persist_scorer(ret_hu_conv);
				const result: bigint = ret == null ? 0n : ret.clone_ptr();
				return result;
			},
	 ...
	 * Persist the given ['ChannelManager'] to disk, returning an error if persistence failed.
	 */
	public persist_manager(channel_manager: ChannelManager): Result_NoneIOErrorZ {
	 ...
	 * Persist the given [`NetworkGraph`] to disk, returning an error if persistence failed.
	 */
	public persist_graph(network_graph: NetworkGraph): Result_NoneIOErrorZ {
	 ...
	 * Persist the given [`WriteableScore`] to disk, returning an error if persistence failed.
	 */
	public persist_scorer(scorer: WriteableScore): Result_NoneIOErrorZ {
