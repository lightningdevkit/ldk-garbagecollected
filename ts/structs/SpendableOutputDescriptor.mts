...
export class SpendableOutputDescriptor extends CommonBase {
	...
	public static constructor_static_output(outpoint: OutPoint, output: TxOut, channel_keys_id: Uint8Array): SpendableOutputDescriptor {
		const ret: bigint = bindings.SpendableOutputDescriptor_static_output(outpoint == null ? 0n : CommonBase.get_ptr_of(outpoint), CommonBase.get_ptr_of(output), bindings.encodeUint8Array(bindings.check_arr_len(channel_keys_id, 32)));
		const ret_hu_conv: SpendableOutputDescriptor = SpendableOutputDescriptor.constr_from_ptr(ret);
	...
	public eq(b: SpendableOutputDescriptor): boolean {
		const ret: boolean = bindings.SpendableOutputDescriptor_eq(this.ptr, b == null ? 0n : CommonBase.get_ptr_of(b));
		return ret;
	}
