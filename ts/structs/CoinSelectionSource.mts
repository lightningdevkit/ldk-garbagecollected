...
export interface CoinSelectionSourceInterface {
	 ...
	 */
	sign_tx(tx: Uint8Array): Result_TransactionNoneZ;
}

class LDKCoinSelectionSourceHolder {
	held: CoinSelectionSource|null = null;
}
...
export class CoinSelectionSource extends CommonBase {
	/* @internal */
	public bindings_instance: bindings.LDKCoinSelectionSource|null;
	...
	public static new_impl(arg: CoinSelectionSourceInterface): CoinSelectionSource {
		const impl_holder: LDKCoinSelectionSourceHolder = new LDKCoinSelectionSourceHolder();
		let structImplementation = {
			select_confirmed_utxos (claim_id: number, must_spend: number, must_pay_to: number, target_feerate_sat_per_1000_weight: number): bigint {
				...
				const ret: Result_CoinSelectionNoneZ = arg.select_confirmed_utxos(claim_id_conv, must_spend_conv_7_arr, must_pay_to_conv_7_arr, target_feerate_sat_per_1000_weight);
			...
			sign_tx (tx: number): bigint {
				...
				const ret: Result_TransactionNoneZ = arg.sign_tx(tx_conv);
				const result: bigint = ret == null ? 0n : ret.clone_ptr();
				return result;
			},
		} as bindings.LDKCoinSelectionSource;
		const ptr_idx: [bigint, number] = bindings.LDKCoinSelectionSource_new(structImplementation);
	 ...
	 * set of other claims being double spent to a minimum.
	 */
	public select_confirmed_utxos(claim_id: Uint8Array, must_spend: Input[], must_pay_to: TxOut[], target_feerate_sat_per_1000_weight: number): Result_CoinSelectionNoneZ {
		const ret: bigint = bindings.CoinSelectionSource_select_confirmed_utxos(this.ptr, bindings.encodeUint8Array(bindings.check_arr_len(claim_id, 32)), bindings.encodeUint64Array(must_spend != null ? must_spend.map(must_spend_conv_7 => must_spend_conv_7 == null ? 0n : CommonBase.get_ptr_of(must_spend_conv_7)) : null), bindings.encodeUint64Array(must_pay_to != null ? must_pay_to.map(must_pay_to_conv_7 => CommonBase.get_ptr_of(must_pay_to_conv_7)) : null), target_feerate_sat_per_1000_weight);
		const ret_hu_conv: Result_CoinSelectionNoneZ = Result_CoinSelectionNoneZ.constr_from_ptr(ret);
		must_spend.forEach((must_spend_conv_7: Input) => { CommonBase.add_ref_from(this, must_spend_conv_7); });
		return ret_hu_conv;
	}
	 ...
	 * trait (i.e., any provided via [`CoinSelectionSource::select_confirmed_utxos`]).
	 */
	public sign_tx(tx: Uint8Array): Result_TransactionNoneZ {
		const ret: bigint = bindings.CoinSelectionSource_sign_tx(this.ptr, bindings.encodeUint8Array(tx));
