...
export class Route extends CommonBase {
	...
	public set_paths(val: Path[]): void {
		bindings.Route_set_paths(this.ptr, bindings.encodeUint64Array(val != null ? val.map(val_conv_6 => val_conv_6 == null ? 0n : CommonBase.get_ptr_of(val_conv_6)) : null));
		val.forEach((val_conv_6: Path) => { CommonBase.add_ref_from(this, val_conv_6); });
	}
	 ...
	 */
	public set_payment_params(val: PaymentParameters|null): void {
	 ...
	 */
	public static constructor_new(paths_arg: Path[], payment_params_arg: PaymentParameters): Route {
		const ret: bigint = bindings.Route_new(bindings.encodeUint64Array(paths_arg != null ? paths_arg.map(paths_arg_conv_6 => paths_arg_conv_6 == null ? 0n : CommonBase.get_ptr_of(paths_arg_conv_6)) : null), payment_params_arg == null ? 0n : CommonBase.get_ptr_of(payment_params_arg));
		const ret_hu_conv: Route = new Route(null, ret);
		CommonBase.add_ref_from(ret_hu_conv, ret_hu_conv);
		paths_arg.forEach((paths_arg_conv_6: Path) => { CommonBase.add_ref_from(ret_hu_conv, paths_arg_conv_6); });
