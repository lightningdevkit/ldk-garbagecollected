...
export class CoinSelection extends CommonBase {
	...
	public set_confirmed_utxos(val: Utxo[]): void {
		bindings.CoinSelection_set_confirmed_utxos(this.ptr, bindings.encodeUint64Array(val != null ? val.map(val_conv_6 => val_conv_6 == null ? 0n : CommonBase.get_ptr_of(val_conv_6)) : null));
		val.forEach((val_conv_6: Utxo) => { CommonBase.add_ref_from(this, val_conv_6); });
	}
	...
	public static constructor_new(confirmed_utxos_arg: Utxo[], change_output_arg: Option_TxOutZ): CoinSelection {
		const ret: bigint = bindings.CoinSelection_new(bindings.encodeUint64Array(confirmed_utxos_arg != null ? confirmed_utxos_arg.map(confirmed_utxos_arg_conv_6 => confirmed_utxos_arg_conv_6 == null ? 0n : CommonBase.get_ptr_of(confirmed_utxos_arg_conv_6)) : null), CommonBase.get_ptr_of(change_output_arg));
		const ret_hu_conv: CoinSelection = new CoinSelection(null, ret);
		CommonBase.add_ref_from(ret_hu_conv, ret_hu_conv);
		confirmed_utxos_arg.forEach((confirmed_utxos_arg_conv_6: Utxo) => { CommonBase.add_ref_from(ret_hu_conv, confirmed_utxos_arg_conv_6); });
		CommonBase.add_ref_from(ret_hu_conv, change_output_arg);
		return ret_hu_conv;
	}
