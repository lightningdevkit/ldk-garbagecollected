...
export class Stfu extends CommonBase {
	...
	public get_channel_id(): Uint8Array {
		const ret: number = bindings.Stfu_get_channel_id(this.ptr);
	...
	public set_channel_id(val: Uint8Array): void {
		bindings.Stfu_set_channel_id(this.ptr, bindings.encodeUint8Array(bindings.check_arr_len(val, 32)));
	}

	/**
	 * Initiator flag, 1 if initiating, 0 if replying to an stfu.
	 */
	public get_initiator(): number {
		const ret: number = bindings.Stfu_get_initiator(this.ptr);
		return ret;
	}
	 ...
	 * Initiator flag, 1 if initiating, 0 if replying to an stfu.
	 */
	public set_initiator(val: number): void {
	 ...
	 */
	public static constructor_new(channel_id_arg: Uint8Array, initiator_arg: number): Stfu {
		const ret: bigint = bindings.Stfu_new(bindings.encodeUint8Array(bindings.check_arr_len(channel_id_arg, 32)), initiator_arg);
	...
	public eq(b: Stfu): boolean {
		const ret: boolean = bindings.Stfu_eq(this.ptr, b == null ? 0n : CommonBase.get_ptr_of(b));
