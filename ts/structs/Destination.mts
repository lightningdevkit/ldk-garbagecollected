...
export class Destination extends CommonBase {
	 ...
	 */
	public static constructor_blinded_path(a: BlindedPath): Destination {
		const ret: bigint = bindings.Destination_blinded_path(a == null ? 0n : CommonBase.get_ptr_of(a));
		const ret_hu_conv: Destination = Destination.constr_from_ptr(ret);
		CommonBase.add_ref_from(ret_hu_conv, ret_hu_conv);
...
export class Destination_BlindedPath extends Destination {
	public blinded_path: BlindedPath;
	/* @internal */
	public constructor(ptr: bigint) {
		super(null, ptr);
		const blinded_path: bigint = bindings.LDKDestination_BlindedPath_get_blinded_path(ptr);
		const blinded_path_hu_conv: BlindedPath = new BlindedPath(null, blinded_path);
