...
export class Wallet extends CommonBase {
	 ...
	 */
	public static constructor_new(source: WalletSource, logger: Logger): Wallet {
		const ret: bigint = bindings.Wallet_new(CommonBase.get_ptr_of(source), CommonBase.get_ptr_of(logger));
		const ret_hu_conv: Wallet = new Wallet(null, ret);
	 ...
	 */
	public as_CoinSelectionSource(): CoinSelectionSource {
		const ret: bigint = bindings.Wallet_as_CoinSelectionSource(this.ptr);
		const ret_hu_conv: CoinSelectionSource = new CoinSelectionSource(null, ret);
