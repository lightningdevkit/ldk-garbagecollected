...
export interface OnionMessageHandlerInterface {
	 ...
	 * connection.
	 */
	get_and_clear_connections_needed(): TwoTuple_PublicKeyCVec_SocketAddressZZ[];
...
export class OnionMessageHandler extends CommonBase {
	...
	public static new_impl(arg: OnionMessageHandlerInterface): OnionMessageHandler {
		...
		let structImplementation = {
			get_and_clear_connections_needed (): number {
				const ret: TwoTuple_PublicKeyCVec_SocketAddressZZ[] = arg.get_and_clear_connections_needed();
				const result: number = bindings.encodeUint64Array(ret.map(ret_conv_40 => ret_conv_40 == null ? 0n : ret_conv_40.clone_ptr()));
				return result;
			},
		...
		const ptr_idx: [bigint, number] = bindings.LDKOnionMessageHandler_new(structImplementation);
	...
	public next_onion_message_for_peer(peer_node_id: Uint8Array): OnionMessage {
		const ret: bigint = bindings.OnionMessageHandler_next_onion_message_for_peer(this.ptr, bindings.encodeUint8Array(bindings.check_arr_len(peer_node_id, 33)));
	...
	public peer_connected(their_node_id: Uint8Array, init: Init, inbound: boolean): Result_NoneNoneZ {
		const ret: bigint = bindings.OnionMessageHandler_peer_connected(this.ptr, bindings.encodeUint8Array(bindings.check_arr_len(their_node_id, 33)), init == null ? 0n : CommonBase.get_ptr_of(init), inbound);
	...
	public provided_node_features(): NodeFeatures {
		const ret: bigint = bindings.OnionMessageHandler_provided_node_features(this.ptr);
	...
	public provided_init_features(their_node_id: Uint8Array): InitFeatures {
		const ret: bigint = bindings.OnionMessageHandler_provided_init_features(this.ptr, bindings.encodeUint8Array(bindings.check_arr_len(their_node_id, 33)));
