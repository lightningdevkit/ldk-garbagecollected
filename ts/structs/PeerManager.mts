...
export class PeerManager extends CommonBase {
	 ...
	 */
	public get_peer_node_ids(): TwoTuple_PublicKeyCOption_SocketAddressZZ[] {
		const ret: number = bindings.PeerManager_get_peer_node_ids(this.ptr);
		const ret_conv_43_len: number = bindings.getArrayLength(ret);
		const ret_conv_43_arr: TwoTuple_PublicKeyCOption_SocketAddressZZ[] = new Array(ret_conv_43_len).fill(null);
		for (var r = 0; r < ret_conv_43_len; r++) {
			const ret_conv_43: bigint = bindings.getU64ArrayElem(ret, r);
			const ret_conv_43_hu_conv: TwoTuple_PublicKeyCOption_SocketAddressZZ = new TwoTuple_PublicKeyCOption_SocketAddressZZ(null, ret_conv_43);
	...
	public broadcast_node_announcement(rgb: Uint8Array, alias: Uint8Array, addresses: SocketAddress[]): void {
		...
		addresses.forEach((addresses_conv_15: SocketAddress) => { CommonBase.add_ref_from(this, addresses_conv_15); });
	}
