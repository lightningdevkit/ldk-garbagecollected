...
export interface NodeSignerInterface {
	 ...
	 * Errors if the [`Recipient`] variant is not supported by the implementation.
	 */
	sign_invoice(hrp_bytes: Uint8Array, invoice_data: UInt5[], recipient: Recipient): Result_RecoverableSignatureNoneZ;
...
export class NodeSigner extends CommonBase {
	...
	public static new_impl(arg: NodeSignerInterface): NodeSigner {
		...
		let structImplementation = {
			...
			sign_invoice (hrp_bytes: number, invoice_data: number, recipient: Recipient): bigint {
				...
				const ret: Result_RecoverableSignatureNoneZ = arg.sign_invoice(hrp_bytes_conv, invoice_data_conv_7_arr, recipient);
				const result: bigint = ret == null ? 0n : ret.clone_ptr();
				return result;
			},
			sign_bolt12_invoice_request (invoice_request: bigint): bigint {
				const invoice_request_hu_conv: UnsignedInvoiceRequest = new UnsignedInvoiceRequest(null, invoice_request);
				const ret: Result_SchnorrSignatureNoneZ = arg.sign_bolt12_invoice_request(invoice_request_hu_conv);
				const result: bigint = ret == null ? 0n : ret.clone_ptr();
				return result;
			},
	...
	public get_inbound_payment_key_material(): Uint8Array {
		const ret: number = bindings.NodeSigner_get_inbound_payment_key_material(this.ptr);
	...
	public ecdh(recipient: Recipient, other_key: Uint8Array, tweak: Option_BigEndianScalarZ): Result_ThirtyTwoBytesNoneZ {
		const ret: bigint = bindings.NodeSigner_ecdh(this.ptr, recipient, bindings.encodeUint8Array(bindings.check_arr_len(other_key, 33)), CommonBase.get_ptr_of(tweak));
		const ret_hu_conv: Result_ThirtyTwoBytesNoneZ = Result_ThirtyTwoBytesNoneZ.constr_from_ptr(ret);
		CommonBase.add_ref_from(this, tweak);
		return ret_hu_conv;
	}
	 ...
	 * Errors if the [`Recipient`] variant is not supported by the implementation.
	 */
	public sign_invoice(hrp_bytes: Uint8Array, invoice_data: UInt5[], recipient: Recipient): Result_RecoverableSignatureNoneZ {
		const ret: bigint = bindings.NodeSigner_sign_invoice(this.ptr, bindings.encodeUint8Array(hrp_bytes), bindings.encodeUint8Array(invoice_data != null ? bindings.uint5ArrToBytes(invoice_data) : null), recipient);
	 ...
	 * [`TaggedHash`]: crate::offers::merkle::TaggedHash
	 */
	public sign_bolt12_invoice_request(invoice_request: UnsignedInvoiceRequest): Result_SchnorrSignatureNoneZ {
		const ret: bigint = bindings.NodeSigner_sign_bolt12_invoice_request(this.ptr, invoice_request == null ? 0n : CommonBase.get_ptr_of(invoice_request));
	...
	public sign_bolt12_invoice(invoice: UnsignedBolt12Invoice): Result_SchnorrSignatureNoneZ {
		const ret: bigint = bindings.NodeSigner_sign_bolt12_invoice(this.ptr, invoice == null ? 0n : CommonBase.get_ptr_of(invoice));
		const ret_hu_conv: Result_SchnorrSignatureNoneZ = Result_SchnorrSignatureNoneZ.constr_from_ptr(ret);
