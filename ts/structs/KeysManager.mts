...
export class KeysManager extends CommonBase {
	 ...
	 */
	public sign_spendable_outputs_psbt(descriptors: SpendableOutputDescriptor[], psbt: Uint8Array): Result_PartiallySignedTransactionNoneZ {
		const ret: bigint = bindings.KeysManager_sign_spendable_outputs_psbt(this.ptr, bindings.encodeUint64Array(descriptors != null ? descriptors.map(descriptors_conv_27 => CommonBase.get_ptr_of(descriptors_conv_27)) : null), bindings.encodeUint8Array(psbt));
		const ret_hu_conv: Result_PartiallySignedTransactionNoneZ = Result_PartiallySignedTransactionNoneZ.constr_from_ptr(ret);
		descriptors.forEach((descriptors_conv_27: SpendableOutputDescriptor) => { CommonBase.add_ref_from(this, descriptors_conv_27); });
		return ret_hu_conv;
	}
	 ...
	 * this [`KeysManager`] or one of the [`InMemorySigner`] created by this [`KeysManager`].
	 */
	public spend_spendable_outputs(descriptors: SpendableOutputDescriptor[], outputs: TxOut[], change_destination_script: Uint8Array, feerate_sat_per_1000_weight: number, locktime: Option_PackedLockTimeZ): Result_TransactionNoneZ {
		...
		descriptors.forEach((descriptors_conv_27: SpendableOutputDescriptor) => { CommonBase.add_ref_from(this, descriptors_conv_27); });
		CommonBase.add_ref_from(this, locktime);
		return ret_hu_conv;
	}
