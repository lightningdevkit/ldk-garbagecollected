...
export interface WalletSourceInterface {
	 ...
	 * attempt.
	 */
	get_change_script(): Result_ScriptNoneZ;
...
export class WalletSource extends CommonBase {
	/* @internal */
	public bindings_instance: bindings.LDKWalletSource|null;
	...
	public static new_impl(arg: WalletSourceInterface): WalletSource {
		const impl_holder: LDKWalletSourceHolder = new LDKWalletSourceHolder();
		let structImplementation = {
			...
			get_change_script (): bigint {
				const ret: Result_ScriptNoneZ = arg.get_change_script();
				const result: bigint = ret == null ? 0n : ret.clone_ptr();
				return result;
			},
		...
		} as bindings.LDKWalletSource;
		const ptr_idx: [bigint, number] = bindings.LDKWalletSource_new(structImplementation);
	...
	public list_confirmed_utxos(): Result_CVec_UtxoZNoneZ {
		const ret: bigint = bindings.WalletSource_list_confirmed_utxos(this.ptr);
	 ...
	 * attempt.
	 */
	public get_change_script(): Result_ScriptNoneZ {
		const ret: bigint = bindings.WalletSource_get_change_script(this.ptr);
	...
	public sign_tx(tx: Uint8Array): Result_TransactionNoneZ {
		const ret: bigint = bindings.WalletSource_sign_tx(this.ptr, bindings.encodeUint8Array(tx));
