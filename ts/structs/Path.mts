...
export class Path extends CommonBase {
	...
	public set_hops(val: RouteHop[]): void {
		bindings.Path_set_hops(this.ptr, bindings.encodeUint64Array(val != null ? val.map(val_conv_10 => val_conv_10 == null ? 0n : CommonBase.get_ptr_of(val_conv_10)) : null));
		val.forEach((val_conv_10: RouteHop) => { CommonBase.add_ref_from(this, val_conv_10); });
	}
	 ...
	 */
	public static constructor_new(hops_arg: RouteHop[], blinded_tail_arg: BlindedTail): Path {
		const ret: bigint = bindings.Path_new(bindings.encodeUint64Array(hops_arg != null ? hops_arg.map(hops_arg_conv_10 => hops_arg_conv_10 == null ? 0n : CommonBase.get_ptr_of(hops_arg_conv_10)) : null), blinded_tail_arg == null ? 0n : CommonBase.get_ptr_of(blinded_tail_arg));
		const ret_hu_conv: Path = new Path(null, ret);
		CommonBase.add_ref_from(ret_hu_conv, ret_hu_conv);
		hops_arg.forEach((hops_arg_conv_10: RouteHop) => { CommonBase.add_ref_from(ret_hu_conv, hops_arg_conv_10); });
		CommonBase.add_ref_from(ret_hu_conv, blinded_tail_arg);
		return ret_hu_conv;
	}
	...
	public eq(b: Path): boolean {
		const ret: boolean = bindings.Path_eq(this.ptr, b == null ? 0n : CommonBase.get_ptr_of(b));
