...
export interface ConfirmInterface {
	 ...
	 */
	get_relevant_txids(): TwoTuple_ThirtyTwoBytesCOption_ThirtyTwoBytesZZ[];
}
...
export class Confirm extends CommonBase {
	...
	public static new_impl(arg: ConfirmInterface): Confirm {
		...
		let structImplementation = {
			...
			get_relevant_txids (): number {
				const ret: TwoTuple_ThirtyTwoBytesCOption_ThirtyTwoBytesZZ[] = arg.get_relevant_txids();
	...
	public transactions_confirmed(header: Uint8Array, txdata: TwoTuple_usizeTransactionZ[], height: number): void {
		bindings.Confirm_transactions_confirmed(this.ptr, bindings.encodeUint8Array(bindings.check_arr_len(header, 80)), bindings.encodeUint64Array(txdata.map(txdata_conv_28 => txdata_conv_28 != null ? CommonBase.get_ptr_of(txdata_conv_28) : 0n)), height);
	}
