...
export class ChainMonitor extends CommonBase {
	...
	public get_claimable_balances(ignored_channels: ChannelDetails[]): Balance[] {
		const ret: number = bindings.ChainMonitor_get_claimable_balances(this.ptr, bindings.encodeUint64Array(ignored_channels.map(ignored_channels_conv_16 => ignored_channels_conv_16 == null ? 0n : CommonBase.get_ptr_of(ignored_channels_conv_16))));
	 ...
	 */
	public list_pending_monitor_updates(): TwoTuple_OutPointCVec_MonitorUpdateIdZZ[] {
		const ret: number = bindings.ChainMonitor_list_pending_monitor_updates(this.ptr);
		const ret_conv_41_len: number = bindings.getArrayLength(ret);
		const ret_conv_41_arr: TwoTuple_OutPointCVec_MonitorUpdateIdZZ[] = new Array(ret_conv_41_len).fill(null);
		for (var p = 0; p < ret_conv_41_len; p++) {
			const ret_conv_41: bigint = bindings.getU64ArrayElem(ret, p);
			const ret_conv_41_hu_conv: TwoTuple_OutPointCVec_MonitorUpdateIdZZ = new TwoTuple_OutPointCVec_MonitorUpdateIdZZ(null, ret_conv_41);
			CommonBase.add_ref_from(ret_conv_41_hu_conv, this);
