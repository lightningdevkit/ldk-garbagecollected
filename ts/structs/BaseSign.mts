	...
	static new_impl(arg: BaseSignInterface, pubkeys: ChannelPublicKeys): BaseSign {
		...
		let structImplementation = {
			...
			validate_holder_commitment (holder_tx: number): number {
				...
				const ret: Result_NoneNoneZ = arg.validate_holder_commitment(holder_tx_hu_conv);
			...
			sign_counterparty_commitment (commitment_tx: number): number {
				...
				const ret: Result_C2Tuple_SignatureCVec_SignatureZZNoneZ = arg.sign_counterparty_commitment(commitment_tx_hu_conv);
				const result: number = ret == null ? 0 : ret.clone_ptr();
				return result;
			},
			...
			sign_channel_announcement (msg: number): number {
				...
				const ret: Result_SignatureNoneZ = arg.sign_channel_announcement(msg_hu_conv);
				const result: number = ret == null ? 0 : ret.clone_ptr();
				return result;
			},
